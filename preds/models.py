...
class FMNISTNet(nn.Sequential):
    """
    Deepobs network with optional last sigmoid activation (instead of relu)
    called `net_mnist_2c2d`
    """

    def __init__(self, use_tanh=False, in_channels=1, n_out=10, in_pixels=28):
        """Args:
        ...
        self.add_module('conv1', tfconv2d(
            in_channels=in_channels, out_channels=32, kernel_size=5, tf_padding_type='same'))
        self.add_module('relu1', nn.ReLU())
        self.add_module('max_pool1', tfmaxpool2d(
            kernel_size=2, stride=2, tf_padding_type='same'))

        self.add_module('conv2', tfconv2d(
            in_channels=32, out_channels=64, kernel_size=5, tf_padding_type='same'))
        self.add_module('relu2', nn.ReLU())
        self.add_module('max_pool2', tfmaxpool2d(
            kernel_size=2, stride=2, tf_padding_type='same'))

        self.add_module('flatten', nn.Flatten())

        in_feats = int(in_pixels / 4)  # 2 times max-pooled with size 2
        self.add_module('dense1', nn.Linear(
            in_features=in_feats * in_feats * 64, out_features=1024))
        if use_tanh:
            self.add_module('relu3', nn.Tanh())
        else:
            self.add_module('relu3', nn.ReLU())

        self.add_module('dense2', nn.Linear(in_features=1024, out_features=n_out))
